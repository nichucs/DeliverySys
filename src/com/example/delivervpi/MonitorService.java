...
//	private  Context mContext=this;
	...
	public void stopUsingGPS(){
		if(locationManager != null){
			locationManager.removeUpdates(MonitorService.this);
		}		
	}
...
//		Toast.makeText(getApplicationContext(), "Location changed:Lat-"+location.getLatitude()+" Long-"+location.getLongitude()+" Time-"+location.getTime(), Toast.LENGTH_LONG).show();
	...
	private void upload(double lat, double lng) {
		SimpleDateFormat date_format=new SimpleDateFormat("yyyy-MM-dd kk:mm:ss",Locale.getDefault());
		ArrayList<NameValuePair> nvp2=new ArrayList<NameValuePair>();
		nvp2.add(new BasicNameValuePair("uid",new SessionManager(getApplicationContext()).getUid()));
		nvp2.add(new BasicNameValuePair("date",""+date_format.format( Calendar.getInstance(Locale.getDefault()).getTime())));
		nvp2.add(new BasicNameValuePair("lat",""+lat));
		nvp2.add(new BasicNameValuePair("lng",""+lng));
		new MyStringAsync(getApplicationContext(), nvp2).execute("http://zappiertech.com/route/1.7/php/controller/?action=setMyLocation");
